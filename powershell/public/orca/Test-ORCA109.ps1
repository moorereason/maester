...
function Test-ORCA109{
    ...
    if($testResult){
        $resultMarkdown += "Well done. Senders are not being allow listed in an unsafe manner`n`n%ResultDetail%"
    }else{
        $resultMarkdown += "Your tenant did not pass. Remove allow listing on senders`n`n%ResultDetail%"
    }

...
function Test-ORCA229{
    ...
    $resultMarkdown = "Microsoft Defender for Office 365 Policies - Anti-phishing trusted domains - 229`n`n"
    if($testResult){
        $resultMarkdown += "Well done. No trusted domains in Anti-phishing policy`n`n%ResultDetail%"
    }else{
        $resultMarkdown += "Your tenant did not pass. Remove allow listing on domains in Anti-phishing policy`n`n%ResultDetail%"
    }

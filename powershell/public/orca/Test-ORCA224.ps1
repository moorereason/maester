...
function Test-ORCA224{
    ...
    $resultMarkdown = "Microsoft Defender for Office 365 Policies - Similar Users Safety Tips - 224`n`n"
    if($testResult){
        $resultMarkdown += "Well done. Similar Users Safety Tips is enabled`n`n%ResultDetail%"
    }else{
        $resultMarkdown += "Your tenant did not pass. Enable Similar Users Safety Tips so that users can receive visible indication on incoming messages`n`n%ResultDetail%"
    }

...
function Test-ORCA238{
    ...
    $resultMarkdown = "Microsoft Defender for Office 365 Policies - Safe Links protections for links in office documents - 238`n`n"
    if($testResult){
        $resultMarkdown += "Well done. Safe Links is enabled for office documents`n`n%ResultDetail%"
    }else{
        $resultMarkdown += "Your tenant did not pass. Enable Safe Links policy action for unknown potentially malicious URLs in office documents`n`n%ResultDetail%"
    }

...
function Test-ORCA237{
    ...
    $obj = New-Object -TypeName ORCA237
    $obj.Run($Collection)
    $testResult = ($obj.Completed -and $obj.ResultStandard -eq "Pass")

    $resultMarkdown = "Microsoft Defender for Office 365 Policies - Safe Links protections for links in teams messages - 237`n`n"
    if($testResult){
        $resultMarkdown += "Well done. Safe Links is enabled for teams messages`n`n%ResultDetail%"
    }else{
        $resultMarkdown += "Your tenant did not pass. Enable Safe Links policy action for unknown potentially malicious URLs in teams messages`n`n%ResultDetail%"
    }

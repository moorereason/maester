...
function Test-ORCA225{
    ...
    $resultMarkdown = "Microsoft Defender for Office 365 Policies - Safe Documents for Office clients - 225`n`n"
    if($testResult){
        $resultMarkdown += "Well done. Safe Documents is enabled for Office clients`n`n%ResultDetail%"
    }else{
        $resultMarkdown += "Your tenant did not pass. Enable Safe Documents for Office clients`n`n%ResultDetail%"
    }
